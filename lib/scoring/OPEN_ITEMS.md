...
| 3 | **Activity `text → tier` (1–4)** | scoring **consumes** an assigned tier per activity; assignment is upstream (schema `Activity.tier`, nullable) | `ec.ts` | decide the assignment method: self-tag / taxonomy pick / model classification |
